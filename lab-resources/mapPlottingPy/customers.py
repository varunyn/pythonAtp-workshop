...
connection = cx_Oracle.connect(
    conn_info['user'], conn_info['password'], conn_info['service_name'])
    ...
    for provider_address, cust_city, cust_address, provider_geometry, cust_geometry, state, miles in cursor.execute(nearest_neighbour_query):
        ...
        if miles < 10:
            nearest_neighbour_data.append(
                Feature(
                    ...
                    properties=({"customer_city": cust_city, "state": state, "color": "red",
                                 "distance": miles, "address": provider_address, "range": "Less than 10 miles"})
                )
            )
            ...
            nearest_neighbour_data.append(
                Feature(
                    ...
                    properties=({"customer_city": cust_city, "state": state, "color": "blue",
                                 "distance": miles, "address": provider_address, "range": "10-200 miles"})
                )
            )
        else:
            nearest_neighbour_data.append(
                Feature(
                    ...
                    properties=({"customer_city": cust_city, "state": state, "color": "black",
                                 "distance": miles, "address": provider_address, "range": "More than 200 miles"})
                )
            )
        ...
        nearest_neighbour_data.append(
            Feature(
                ...
                properties=({"address": cust_address,
                             "color": "green", "range": "Service Providers"})
            )
        )
except cx_Oracle.DatabaseError as exc:
    # <class 'cx_Oracle.NotSupportedError'>
    print(str(type(exc)))
    # NotSupportedError('Variable_TypeByValue(): unhandled data type dict',)
    print(repr(exc))
    # "error" is a str, NOT a cx_Oracle._Error object
    error, = exc.args
...
location = gpd.GeoDataFrame(df, columns=[
                            'customer_city', 'address', 'geometry', 'state', 'distance', 'color', 'range'])
...
location['x'] = location.apply(
    getPointCoords, geom='geometry', coord_type='x', axis=1)
location['y'] = location.apply(
    getPointCoords, geom='geometry', coord_type='y', axis=1)
...
palette = palette[::-1]
color_mapper_age = LinearColorMapper(
    palette=palette, low=0, high=90, nan_color='#d9d9d9')
color_mapper_income = LinearColorMapper(
    palette=palette, low=0, high=100000, nan_color='#d9d9d9')

# CREATE FIGURE

# ADD HOVER TOOL
my_hover = HoverTool(names=["states"])
my_hover.tooltips = [('Nearest service provider:',
...
p.circle(x='x', y='y', size='sizes', source=source, line_color="color",
         fill_color="color", fill_alpha=0.05, name="states", legend='range')
